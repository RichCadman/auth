...
class DbManage {
     ...
     */
    function __construct($host = 'localhost', $username = 'root', $password = 'root', $database = 'export', $charset = 'utf8') {
        $this->host = $host;
        ...
        if (! empty ( $tablename )) {
            ...
            while ( $table = mysql_fetch_array ( $tables ) ) {
                ...
                while ( $record = mysql_fetch_array ( $data ) ) {
                    // 单条记录
                    $sql .= $this->_insert_record ( $tablename, $num_fields, $record );
                    //$this->_showMsg($size * 1000);exit;
                    // 如果大于分卷大小，则写入多个文件：20120516211738_all_v1.sql，20120516211738_all_v2.sql
    ...
    function restore($sqlfile) {
        ...
        //$this->_showMsg(var_dump($sqlpath));exit;
        $this->sqldir = $sqlpath ['dirname'];
        //$this->_showMsg(var_dump($this->sqldir));exit;
        // 检测是否包含分卷，将类似 _v1.sql从_v分开,有则说明有分卷
        $volume = explode ( "_v", $sqlfile );
        //$this->_showMsg(var_dump($volume));exit;
        $volume_path = $volume [0];//得到格式为：20120516211738_all
        //$this->_showMsg(var_dump($volume_path));exit;
        ...
        if (empty ( $volume [1] )) {
            ...
            // 存在分卷，则获取当前是第几分卷，循环执行余下分卷   $volume [1]=>"1.sql"
            $volume_id = explode ( ".sq", $volume [1] );//array(2) { [0]=> string(1) "1" [1]=> string(1) "l" }
            //$this->_showMsg(var_dump($volume_id));exit;
            // 当前分卷为$volume_id
            $volume_id = intval ( $volume_id [0] );
            while ( $volume_id ) {
                $tmpfile = $volume_path . "_v" . $volume_id . ".sql";//20120516211738_all_v1.sql
                // 存在其他分卷，继续执行
